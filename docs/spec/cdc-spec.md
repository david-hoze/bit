...
```haskell
-- Bit/CDC/Gear.hs
-- Generated by: for i in 0..255, take 8 bytes of SHA-256(uint64_le(i))
-- interpreted as little-endian Word64.
gearTable :: Vector Word64
gearTable = V.fromList
  [ 0x..., 0x..., ... ]  -- 256 entries, generated at build time
```
...
[cdc]
    enabled = true        # enable content-defined chunking (default: unset/disabled)
...
-- SHA-256(i) interpreted as little-endian Word64, for i in [0..255].
-- This is a compile-time constant.
gearTable :: Vector Word64
gearTable = V.fromList [ {- 256 literal Word64 values -} ]
```

The table is generated once (by a script or in a comment) and committed as a literal. Deterministic chunking requires all implementations to use the exact same table.
